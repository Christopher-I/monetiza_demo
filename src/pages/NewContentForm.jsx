...
const ContentForm = () => {
  ...
  const defaultDate = currentDate.toISOString().split("T")[0]; // YYYY-MM-DD format
  const defaultTime = `${currentDate.getHours().toString().padStart(2, '0')}:${currentDate.getMinutes().toString().padStart(2, '0')}`;
  ...
  const draftPost = async () => {
    ...
    try {
      ...
      formData.append("publishDate", `${date}T${time}`)
      formData.append("caption", stripHtmlTags(caption) === "undefined" ? "" : stripHtmlTags(caption))
      formData.append("image", image)
      const response = await axios.post(`${baseUrl}/api/post/draft`, formData, {
        // {...formData, ...uploadData}, {
    ...
    <div className="flex-1 bg-white px-8 rounded-lg shadow-md flex lg:grid flex-col lg:grid-cols-3 gap-4 overflow-y-auto max-h-screen pb-0">
      ...
      <div className="flex flex-col space-y-6 pt-8">
        ...
        <div className='flex flex-col'>
          ...
          <input type="date" className="border p-3 rounded-lg w-3/4 mt-2" onChange={(e) => setDate(e.target.value)} />
        </div>

        {/* Post Time */}
        <div className='flex flex-col'>
          <label className="text-gray-700">Post Time</label>
          <input type="time" className="border p-3 rounded-lg w-3/4 mt-2" onChange={(e) => setTime(e.target.value)} />
        </div>
