...
const Post = () => {
    ...
    const fetchComments = async () => {
        try {
            const result = await axios.post(`${baseUrl}/api/post/${id}/comment/all`, {}, {
    ...
    const fetchPost = async () => {
        try {
            const result = await axios.get(`${baseUrl}/api/post/${id}`, {
                headers: { "Content-Type": "application/json" },
                withCredentials: true,
            });
    ...
    const addComment = async () => {
        ...
        try {
            await axios.post(`${baseUrl}/api/post/${id}/comment`, { text, gif: selectedGif }, {
                headers: { "Content-Type": "application/json" },
                withCredentials: true,
            });
    ...
    return (
        <div className={`flex overflow-y-auto hide-scrollbar pb-[70px] lg:pb-0 max-h-screen h-screen flex-1 ${isMobile ? "flex-col" : "lg:flex-row"}`}>
            <div className="w-full flex flex-col h-screen overflow-y-auto relative lg:w-3/4 border-r border-[var(--border-color)]">
...
export default withProtectedRoute(Post);
