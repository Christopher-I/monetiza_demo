...
const UnlockContentModal = ({ isOpen, onClose, data, onPaymentSuccess }) => {
  ...
  // Use data.price instead of a separate amount state
  ...
  const handleSendTip = async () => {
    // Use data.price as the payment amount
  ...
  const handleCreatePaypalPayment = async () => {
    if (data.price === 0 || data.price === "") return toast.error("Content price is not set");
    ...
    try {
      ...
      // For PayPal, the user is redirected so we cannot immediately call onPaymentSuccess here.
  ...
  const handleCreateCardPayment = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${baseUrl}/api/payment/charge/saved`, {
        postId: data.postId,
        amount: data.price,
        userId: user._id,
        creatorId: data.authorId,
  ...
  const handleCreateStripePayment = async () => {
    if (data.price === 0) return toast.error("Content price is not set");
    if (!data.authorId) return toast.error("No creator found");
    setLoading(true);
    try {
      ...
      // For Stripe, the user is redirected; unlocking can be handled on the return route.
    } catch (error) {
      console.error(error);
  ...
  const handleBitcoinPayment = async () => {
    if (!data.price || data.price <= 0) {
      return toast.error("Content price is not set");
    }
    if (selectedCard === "bitcoin" && Number(amount) < 20) {
      return toast.error("The minimum Bitcoin payment is $20");
    }
    setLoading(true);
    try {
      ...
      if (response?.data?.success) {
        ...
        // Show the Bitcoin payment modal and pass onPaymentSuccess so that when Bitcoin payment completes,
        // the modal can call the callback to unlock the content.
      ...
      toast.error("Bitcoin payment failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose}>
      <div className="p-4 font-montserrat">
        <div className="text-2xl my-2 font-semibold">Unlock Content</div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div
              ...
              className={`flex flex-col justify-between cursor-pointer border ${selectedCard === name ? "border-[#E35F01]" : "border-gray-300"} rounded-md`}
            >
        ...
        <button
          ...
          className="mx-auto w-full bg-orange-500 text-white p-3 py-1 mt-3 rounded-full hover:bg-orange-600"
        >
          {loading ? "Processing..." : `Unlock Content for $${data.price}`}
        </button>
      </div>
